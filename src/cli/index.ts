  ...
  .action(async (path: string, options: { file?: string }) => {
    ...
    try {
      ...
      await handleOutput(busFactor, options, () => {
        let output = '';
        output += '\n';
        output += chalk.bold.cyan(`  🚌 Bus Factor Analysis`) + '\n';
  ...
  .action(async (path: string, options: { file?: string }) => {
    ...
    try {
      ...
      await handleOutput(velocity, options, () => {
        let output = '';
        output += '\n';
        output += chalk.bold.cyan(`  🚀 Development Velocity`) + '\n';
        output += chalk.gray(`  ${'─'.repeat(50)}`) + '\n';
    ...
    try {
      ...
      if (complexity.godFiles.length > 0) {
        ...
        complexity.godFiles.slice(0, options.top).forEach(file => {
          ...
          console.log(`    ${chalk.red('●')} ${filePath} ${chalk.gray(file.reason)}`);
        });
      }
      ...
      if (complexity.growingFiles.length > 0) {
        ...
        complexity.growingFiles.slice(0, options.top).forEach(file => {
          const filePath = file.path.length > 45 ? '...' + file.path.slice(-42) : file.path.padEnd(45);
          console.log(`    ${chalk.yellow('●')} ${filePath} +${file.netGrowth} lines (${file.trend})`);
        });
      }
      ...
      if (complexity.criticalHotspots && complexity.criticalHotspots.length > 0) {
        ...
        complexity.criticalHotspots.slice(0, options.top).forEach(hotspot => {
          ...
          console.log(`    ${riskColor('●')} ${filePath} Risk: ${riskColor(hotspot.riskScore.toString())}%`);
  ...
  .action(async (path: string, options: { file?: string }) => {
    ...
    try {
      ...
      await handleOutput(burnout, options, () => {
        let output = '';
        output += '\n';
        output += chalk.bold.red(`  🔥 Burnout Risk Analysis`) + '\n';
        output += chalk.gray(`  ${'─'.repeat(50)}`) + '\n';
        ...
        const riskEmoji = burnout.teamRiskLevel === 'critical' ? '🚨' :
                         burnout.teamRiskLevel === 'high' ? '⚠️' :
                         burnout.teamRiskLevel === 'medium' ? '📊' : '✅';
        ...
        if (burnout.developerRisks.length > 0) {
          ...
          burnout.developerRisks.slice(0, 10).forEach(dev => {
            ...
            const emoji = dev.riskLevel === 'critical' ? '🔴' :
                         dev.riskLevel === 'high' ? '🟡' :
            ...
            if (dev.signals.length > 0) {
              ...
              output += `       ${chalk.gray('└─')} ${topSignal.description}\n`;
            }
          });
        }
        ...
        if (tp.crunchPeriodsDetected.length > 0) {
          ...
          tp.crunchPeriodsDetected.slice(0, 3).forEach(crunch => {
            ...
            output += `    ${intensity('●')} ${crunch.startDate.toISOString().split('T')[0]} to ${crunch.endDate.toISOString().split('T')[0]}\n`;
  ...
  .action(async (path: string, options: { top: string; file?: string }) => {
    ...
    try {
      ...
      await handleOutput(stats, options, () => {
        let output = '';
        output += '\n';
        output += chalk.bold.yellow(`  🏆 GITSTATS LEADERBOARDS 🏆`) + '\n';
        output += chalk.gray(`  ${'─'.repeat(50)}`) + '\n';
  ...
  .action(async (path: string, options: { file?: string }) => {
    ...
    try {
      ...
      await handleOutput(stats, options, () => {
        let output = '';
        output += '\n';
        output += chalk.bold.gray(`  💀 Dead Code Detection`) + '\n';
        output += chalk.gray(`  ${'─'.repeat(50)}`) + '\n';
        ...
        if (stats.deprecatedPatterns.length > 0) {
          output += chalk.bold.red(`\n  ⚠️  Deprecated Naming Patterns:`) + '\n';
          stats.deprecatedPatterns.slice(0, 5).forEach(file => {
            output += `    ${chalk.red('●')} ${file.path}\n`;
        ...
        if (stats.testOrphans.length > 0) {
          output += chalk.bold.yellow(`\n  🧪 Orphan Tests (no source file):`) + '\n';
          stats.testOrphans.slice(0, 5).forEach(orphan => {
            output += `    ${chalk.yellow('●')} ${orphan.testFile}\n`;
  ...
  .action(async (path: string, options: { file?: string }) => {
    ...
    try {
      ...
      await handleOutput(stats, options, () => {
        let output = '';
        output += '\n';
        output += chalk.bold.blue(`  🕸️  Dependency Analysis`) + '\n';
        output += chalk.gray(`  ${'─'.repeat(50)}`) + '\n';
        ...
        if (stats.circularDependencies.length > 0) {
          ...
          stats.circularDependencies.slice(0, 3).forEach(cycle => {
            ...
            output += `    ${severity('●')} Cycle of ${cycle.length} files:\n`;
        ...
        if (stats.layerViolations.length > 0) {
          ...
          stats.layerViolations.slice(0, 5).forEach(violation => {
            output += `    ${chalk.red('●')} ${violation.fromLayer} → ${violation.toLayer}\n`;
  ...
  .action(async (path: string, options: { file?: string }) => {
    ...
    try {
      ...
      await handleOutput(stats, options, () => {
        let output = '';
        output += '\n';
        output += chalk.bold.magenta(`  📋 Copy-Paste Detection`) + '\n';
        output += chalk.gray(`  ${'─'.repeat(50)}`) + '\n';
        ...
        if (stats.similarFiles.length > 0) {
          ...
          stats.similarFiles.slice(0, 5).forEach(pair => {
            ...
            output += `    ${simColor('●')} ${f1}\n`;
            output += `      ↔ ${f2} ${simColor(`(${pair.similarity}% similar)`)}\n`;
          });
        }
        ...
        if (stats.cloneGroups.length > 0) {
          ...
          stats.cloneGroups.slice(0, 5).forEach(group => {
            output += `    ${chalk.red('●')} Clone Group #${group.id} (${group.instances.length} instances, ${group.lines} lines)\n`;
        ...
        if (stats.patternDuplicates.length > 0) {
          ...
          stats.patternDuplicates.slice(0, 3).forEach(pattern => {
            output += `    ${chalk.cyan('●')} ${pattern.pattern}\n`;
  ...
  .action(async (path: string, options: { file?: string }) => {
    ...
    try {
      ...
      await handleOutput(stats, options, () => {
        let output = '';
        output += '\n';
        output += chalk.bold.cyan(`  🏙️  Code City`) + '\n';
        output += chalk.gray(`  ${'─'.repeat(50)}`) + '\n';
        ...
        output += `    ${chalk.green('● Healthy:')}  ${s.healthyBuildings}\n`;
        output += `    ${chalk.yellow('● Warning:')}  ${s.warningBuildings}\n`;
        output += `    ${chalk.red('● Critical:')} ${s.criticalBuildings}\n`;
        ...
        if (stats.city.length > 0) {
          ...
          stats.city.slice(0, 8).forEach(district => {
            ...
            const activity = district.metrics.activityLevel === 'hot' ? '🔥' :
                            district.metrics.activityLevel === 'warm' ? '🌡️' : '❄️';
        ...
        if (tallest.length > 0) {
          ...
          tallest.forEach(b => {
            ...
            output += `    ${bHealthColor('●')} ${bPath} (${b.metrics.loc} LOC)\n`;
          });
        }
...
program
  ...
  .action(async (path: string, options: { dir: string }) => {
    ...
    try {
      ...
      console.log(chalk.green(`    ✓ burnout.html`) + chalk.gray(' - Burnout Risk'));
      ...
      console.log(chalk.green(`    ✓ leaderboard.html`) + chalk.gray(' - Contributor Leaderboard'));
      ...
      console.log(chalk.green(`    ✓ deadcode.html`) + chalk.gray(' - Dead Code Detection'));
      ...
      console.log(chalk.green(`    ✓ dependencies.html`) + chalk.gray(' - Dependency Analysis'));
      ...
      console.log(chalk.green(`    ✓ duplicates.html`) + chalk.gray(' - Code Duplicates'));
      ...
      console.log(chalk.green(`    ✓ city.html`) + chalk.gray(' - 3D Code City'));
...
<body>
  <div class="container">
    ...
    <p class="footer">Generated by GitStats v0.1.3</p>
  </div>
</body>
</html>`;

      await writeFile(`${outputDir}/index.html`, indexHtml);
      console.log(chalk.green(`    ✓ index.html`) + chalk.gray(' - Report Index'));
